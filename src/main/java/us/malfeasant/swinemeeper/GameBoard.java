...
public class GameBoard extends Application {
    ...
    private void newGame() {
        ...
        if (rebuild) {
            // first, without this, board would be built with buttons taking no size at all...
            // so we added sizeToScene() - then board was built with correct size.. 
            // but it was possible to resize board too small, then buttons got ugly
            // so we setMinHeight/Width to its current width as soon as we build the board
            // but then, going from harder to easier leaves min sizes too big...
            // so we have to set to 0 first, then sizeToScene, then set to that.
